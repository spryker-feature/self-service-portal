...
class UnattachedModelAssetAttachmentTable extends AbstractTable
{
    ...
    protected function prepareQuery(): SpySspAssetQuery
    {
        $query = $this->sspAssetQuery
            ->useSpySspAssetToSspModelQuery(SpySspAssetToSspModelTableMap::TABLE_NAME, Criteria::LEFT_JOIN)
                ->filterByFkSspModel(null, Criteria::ISNULL)
            ->endUse()
            ->addJoinCondition(
                SpySspAssetToSspModelTableMap::TABLE_NAME,
                sprintf('%s = %d', SpySspAssetToSspModelTableMap::COL_FK_SSP_MODEL, $this->idSspModel),
            )
            ->select([
