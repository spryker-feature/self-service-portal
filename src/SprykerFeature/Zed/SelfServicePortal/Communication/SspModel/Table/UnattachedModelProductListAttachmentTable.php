...
class UnattachedModelProductListAttachmentTable extends AbstractTable
{
    ...
    protected function prepareQuery(): SpyProductListQuery
    {
        $query = $this->productListQuery
            ->useSpySspModelToProductListQuery(SpySspModelToProductListTableMap::TABLE_NAME, Criteria::LEFT_JOIN)
                ->filterByFkSspModel(null, Criteria::ISNULL)
            ->endUse()
            ->addJoinCondition(
                SpySspModelToProductListTableMap::TABLE_NAME,
                sprintf('%s = %d', SpySspModelToProductListTableMap::COL_FK_SSP_MODEL, $this->idSspModel),
            )
            ->select([
