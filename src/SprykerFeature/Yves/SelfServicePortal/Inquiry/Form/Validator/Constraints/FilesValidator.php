...
class FilesValidator extends FileValidator
{
    ...
    {
        if (!is_array($value)) {
            $this->context->buildViolation('self_service_portal.inquiry.error.file.format.invalid')
                ->addViolation();
        ...
        $this->context->buildViolation('self_service_portal.inquiry.error.file.size.invalid', [
